...
const PatientManagement = () => {
  ...
  const handleViewPatient = async (id) => {
    try {
      const response = await axios.get(`http://44.196.64.110:9007/api/measures/data/${id}`);
  ...
  const generatePdfForPreview = async (userId, date) => {
    try {
      const response = await axios.post(
        'http://44.196.64.110:9007/api/patient/generate-pdf',
        { userId, date },
      ...
      if (response.data instanceof Blob) {
        ...
        // Set the URL to be used for preview
