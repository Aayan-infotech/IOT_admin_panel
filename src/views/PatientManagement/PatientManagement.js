...
const PatientManagement = () => {
  ...
  useEffect(() => {
    const fetchPatients = async () => {
      try {
        const response = await axios.get('http://44.196.64.110:9007/api/patient/');
        ...
        const updatedPatients = await Promise.all(
          patientData.map(async (patient) => {
            if (patient.userId) {
              try {
                const userResponse = await axios.get(`http://44.196.64.110:9007/api/user/${patient.userId}`);
  ...
  const handleViewPatient = async (patient) => {
    try {
      const response = await axios.get(`http://44.196.64.110:9007/api/patient/getPatients/${patient.userId}`);
  ...
  const handleAddPatient = async (event) => {
    ...
    try {
      const response = await axios.post('http://44.196.64.110:9007/api/auth/register', formData);
